...
## Header
echo "filename;filename_core;metadata_filename;param;type;offset;slope;drift;param_error;qc;scientific_comment;date_update"  >> "DM_list_"$id_WMO
...
case "$id_LIST" in
	"0")
		...
		do
			...
			echo $i";"$icore";"$metadata_file";"$PARAM";AD;"$OFFSET";"$SLOPE";"$DRIFT";"$PARAM_ERROR";"$QC";"$SC_COMMENT";"$DATE_UPDATE  >> "DM_list_"$id_WMO
		done  
		;;
	"1")
		...
		do
			...
			do
				...
				then 				
				echo $i";"$icore";"$metadata_file";"$PARAM";"$TYPE";"$OFFSET";"$SLOPE";"$DRIFT";"$PARAM_ERROR";"$QC";"$SC_COMMENT";"$DATE_UPDATE  >> "DM_list_"$id_WMO
				fi
				((a+=1))
...
echo "./lance_DM.sh"
echo "" 
echo "If it is not, go back ./to Fill_DM.sh"
echo " or contact me :"
