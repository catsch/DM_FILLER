###################################################################################################
#	This program is designed to modify a BR file into BD file wit inputs from Xiaogang
#       
#       Catherine Schmechtig 2020  July 	
#
...
#       V2.1 20200723 : Add a correction for NPQ and F490 for CHLA (dedicated to Xiaogang Xing's correction for Takuvik floats) 	      
#
...
#		
#	-Inputs : 
#		output of the script ./Fill_DM_BP_XX.sh
#		Bfiles
#	-Output :
...
###	Next iteration if the parameter is not in the file 
	...
	if ( (PARAM_name[i] == "DOXY") | (CORRECTION_TYPE[i] == "GL") | (PARAM_name[i] == "CHLA") ) {
	...
	# For DOXY we need to go through the adjustment of PPOX_DOXY 
		...
		if ( PARAM_name[i] == "DOXY" ) {
			...
			if ( PARAM_name[i] == "NITRATE" ) {
				...
				if ( PARAM_name[i] == "CHLA" ) {

					CHLA_ADJ=CHLA_adj_XX(filenc_core, filenc, PARAM_name[i] ,OFFSET[i], SLOPE[i], ERROR[i])	

	                                PARAM_ADJUSTED=CHLA_ADJ$CHLA

       		                        PARAM_ADJUSTED_ERROR=CHLA_ADJ$CHLA_ERROR

                                	CHLA_INDEX_NPQ=CHLA_ADJ$CHLA_INDEX_NPQ

					FLAG_QUENCHING=CHLA_ADJ$FLAG_QUENCHING
		...
		for (i_qc in seq(1,N_QC)) {
			...
			if ( PARAM_name[i] != "NITRATE" ) {
				...
				if ( ( QC_test == "3") || ( QC_test == "2") || (QC_test == " ") || ( QC_test == "1") ) { # To check
					...
					if ( PARAM_name[i] == "CHLA" & j <=CHLA_INDEX_NPQ & FLAG_QUENCHING ) {

						substr(PARAM_ADJUSTED_QC[i_prof_param], j , j)<-"5" # Quenching Area 
	...
	if ( CORRECTION_TYPE[i] == "AD") {
		...
		if ( PARAM_name[i] == "DOXY" ) {
			...
			if ( PARAM_name[i] == "CHLA" ) {
				...
				scientific_equation=paste("CHLA_DC=CHLA-OFFSET;CHLA_X12=max(CHLA_DC(DEPTH<min(MLD,DEPTH_ipar15)));DEPTH_X12=DEPTH(CHLA_X12);IF DEPTH>DEPTH_X12,CHLA_ADJUSTED=SLOPE*CHLA_DC;IF DEPTH < DEPTH_X12,CHLA_ADJUSTED=SLOPE*CHLA_X12")
				...
				if ( PARAM_name[i] == "NITRATE" ) {
					...
					scientific_equation=paste("NITRATE_ADJUSTED=NITRATE-(OFFSET+CYCLE_NUMBER*DRIFT)")
